...
namespace Fusion {
	...
	{
	public:
		void Add(const std::string& p_name, Ref<Shader>& p_shader);
		Ref<Shader> GetShader(const std::string& p_name) const;

		bool Exists(const std::string& p_name) const;
		
	private:
		std::unordered_map<std::string, Ref<Shader>> _library;
	};
}